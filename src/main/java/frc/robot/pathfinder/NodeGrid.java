...
public class NodeGrid {
    ...
    private Translation2d GridToFieldPos(int gridX, int gridY) {
        return _field.config.bottomLeftPositionMeters.plus(new Translation2d(gridX, gridY).times(_field.config.nodeSpacingMeters));
    }

    /** @return the closest node to the given position (clamped to be in bounds) */
    public Node FindCloseNode(Translation2d position) {
        int x = (int) Math.round((position.getX() - _field.config.bottomLeftPositionMeters.getX()) / _field.config.nodeSpacingMeters);
        int y = (int) Math.round((position.getY() - _field.config.bottomLeftPositionMeters.getY()) / _field.config.nodeSpacingMeters);
